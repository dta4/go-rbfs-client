...
/*
StreamsApiService Traces the LSP to the given destination IP prefix (FEC).
Traces the LSP to the given destination from the specified source IP or source IFL with the provided settings.  Three options exist to specify the traceroute destination:  1. Use &#x60;destination_ip_fec&#x60; to specify the destination IP address in IPv6 or IPv4 format. 2. Use &#x60;destination_aaaa&#x60; to query the DNS for an AAAA record for the specified host name. 3. Use &#x60;destination_a&#x60; to query the DNS for an A record for the specified host name.  The precedence is in the above ordering if multiple destinations are specified. An error is returned if no destination is specified.
 * @param ctx context.Context - for authentication, logging, cancellation, deadlines, tracing, etc. Passed from http.Request or context.Background().
 * @param optional nil or *StreamsApiStreamTracerouteMplsOpts - Optional Parameters:
     * @param "FecType" (optional.String) -  The FEC type.
     * @param "DestinationIpFec" (optional.String) -  Destination IPv4 or IPv6 prefix.
     * @param "DestinationAaaa" (optional.String) -  Destination hostname to query DNS for an AAAA record (IPv6)
     * @param "DestinationA" (optional.String) -  Destination hostname to query DNS for an A record (IPv4)
     * @param "SourceIp" (optional.String) -  Source IPv4 or IPv6 address
     * @param "InstanceName" (optional.String) -  Routing instance name
     * @param "MaxHops" (optional.Int) -  Maximum number of acceptable hops to reach the given destination.
     * @param "Size" (optional.Int) -  Packet payload size in bytes.
     * @param "WaitTime" (optional.Float64) -  Maximum wait time between pings.
     * @param "Interval" (optional.Float64) -  Ping interval in seconds. DEPRECTATED use wait_time instead.
@return string
*/

type StreamsApiStreamTracerouteMplsOpts struct {
	FecType          optional.String
	DestinationIpFec optional.String
	DestinationAaaa  optional.String
	DestinationA     optional.String
	...
	MaxHops          optional.Int
	Size             optional.Int
	WaitTime         optional.Float64
	Interval         optional.Float64
}

func (a *StreamsApiService) StreamTracerouteMpls(ctx context.Context, localVarOptionals *StreamsApiStreamTracerouteMplsOpts) (string, *http.Response, error) {
	var (
		localVarHttpMethod  = strings.ToUpper("Post")
		localVarPostBody    interface{}
		localVarFileName    string
	...
	// create path and map variables
	localVarPath := a.client.cfg.BasePath + "/streams/traceroute-mpls"
	...
	if localVarOptionals != nil && localVarOptionals.WaitTime.IsSet() {
		localVarQueryParams.Add("wait_time", parameterToString(localVarOptionals.WaitTime.Value(), ""))
	}
	if localVarOptionals != nil && localVarOptionals.Interval.IsSet() {
		localVarQueryParams.Add("interval", parameterToString(localVarOptionals.Interval.Value(), ""))
	}
